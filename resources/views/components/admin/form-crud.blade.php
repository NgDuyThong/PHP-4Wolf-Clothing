...
<div id="form-data" hidden data-rules="{{ json_encode($rules) }}"
  ...
  <div class="card-body">
<form action="{{ $route }}" method="POST" id="form__js" enctype="multipart/form-data" class="row g-3">
  @csrf
  <!-- /.card-header -->
    @foreach ($fields as $field)
    <x-admin-input-prepend label="{{$field['label']}}">
      @if ($field['type'] == 'select')
        <select class="form-select" name="{{$field['attribute']}}" id="{{$field['attribute']}}">
          @if (isset($field['list']))
      ...
      <div class="custom-file">
        <input type="file" name="{{$field['attribute']}}" class="form-control inputFile__js" id="{{$field['attribute']}}">
      </div>
      @else
      <input 
        id="{{$field['attribute']}}"
